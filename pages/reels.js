...
export default function Home (req, res) {
  ...
  const title = 'Instagram Reels Download';
  ...
  const fetchData = async () => {
    try {
      ...
      if (jsonData.id === "nopost") {
       ...
       // Reset error if fetch is successful
    } catch (error) {
...
<div class="card w-full bg-base-100 ">
  ...
  <svg
    aria-label="Verified"
    class="w-4 animate-ping animate-once  animate-delay-[3000ms] animate-ease-in animate-reverse "
    fill="rgb(0, 149, 246)"
    role="img"
    viewBox="0 0 40 40"
  >
