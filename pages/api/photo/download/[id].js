...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fileName = `@${postedby}-photo-${longid}.jpg`;

    // Set headers for the response to trigger download with custom filename
    res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
    res.setHeader('Content-Type', 'image/jpeg'); // Assuming the thumbnail is a JPEG image
    res.send(thumbnailResponse.data); // Send the binary data of the thumbnail
