  ...
  try {
    ...
    const response = await axios.post(API_URL, new URLSearchParams(postData).toString(), {
      headers: headers
        ...
        const postedby = responseData?.data?.xdt_shortcode_media?.owner?.username;
        const videourl = responseData?.data?.xdt_shortcode_media?.video_url;

    // Download the thumbnail with a custom name
    const videourlResponse = await axios.get(videourl, {
      responseType: 'arraybuffer' // Ensure the response is in binary format
    });

    // Set custom filename for the downloaded thumbnail
    const fileName = `@${postedby}-reel-${longid}.mp4`;

    // Set headers for the response to trigger download with custom filename
    res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
    res.setHeader('Content-Type', 'video/mp4');
    res.send(videourlResponse.data); // Send the binary data of the thumbnail
